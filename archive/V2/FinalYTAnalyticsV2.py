...
API_KEY = os.getenv("API_KEY") #Please ask Anna, Frank, Ian, or Lucy for the API_KEY  
channelID = None
...
def get200DaysOfData(startDate='N', endDate='N'):
    format = '%Y-%m-%d'
    report = None
    ...
    report = client.retrieve(start_date=startDate, #data starting date
                ...
                'estimatedRedPartnerRevenue', 'monetizedPlaybacks', 'playbackBasedCpm', 'adImpressions', 'cpm'], #list of metrics to retrieve data for
                max_results=200, # set maximum number of results (has to be less than 200)
                sort_by=['-views'],  #required parameter. sorts by views in descending order (- means descending)
...
def retrieveAllData(startDate='N', endDate='N', vidPublishBefore='N', vidPublishAfter='N'):
    ...
    reportDF = get200DaysOfData(startDate, endDate)
